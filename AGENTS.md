...
- **Phase 4 AI workflows:** `ai-evaluation.yml` direct event mode remains advisory and secret-gated, while reusable deterministic mode runs `--check` without a credential or provider call. `open-code-review.yml` (Alibaba OCR) remains advisory and skips without `OCR_LLM_*` secrets. Both direct workflows use `pull_request` (not `pull_request_target`) per the security rule. See `docs/ai/evaluation-strategy.md`.
