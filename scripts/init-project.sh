...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --project-type)
      ...
      ;;
    --ai-enabled)
      [ "$#" -ge 2 ] || die '--ai-enabled requires a value'
      AI_ENABLED="$2"
      shift 2
      ;;
    --deployment-target)
...
if [ -f "$PROFILE_FILE" ] && [ "$RECONFIGURE" != 'yes' ]; then
  die 'profile config already exists; rerun with --reconfigure to replace it'
fi
...
PROFILE_TEMP="$PROFILE_FILE.init.$$"
trap 'rm -f "$BLOCK_FILE" "$TEMP_FILE" "$CONFIG_TEMP" "$PROFILE_TEMP" "$COMPONENT_ROWS" "$COMPONENT_INPUT"' EXIT HUP INT TERM
...
printf '%s\n' \
  ...
  "  scorecard: $SCORECARD" > "$PROFILE_TEMP"

sh "$(dirname "$0")/validate-project-config.sh" "$CONFIG_TEMP" >/dev/null 2>&1 \
  || die 'generated project config is invalid'
sh "$(dirname "$0")/validate-profile-config.sh" "$PROFILE_TEMP" >/dev/null 2>&1 \
