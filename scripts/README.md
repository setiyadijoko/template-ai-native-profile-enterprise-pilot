...
| `validate-profile-config.sh` | `make profile-config-check` / `make ci` | Validates the credential-free `.template/profile.yaml` maturity-profile contract. Missing config keeps template compatibility mode; an initialized consumer missing a profile fails the policy check. Profile values select reusable jobs in the advisory aggregate; enforcement and branch-protection migration remain staged. |
